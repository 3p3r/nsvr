...
class CompletePlayback : public App {
public:
    void                setup() override;
...
void CompletePlayback::CinderPlayer::onFrame(guchar* buf, gsize size) const
{
    if (mTexture)
        mTexture->update(buf, GL_RGBA, GL_UNSIGNED_BYTE, 0, getWidth(), getHeight());
}

void CompletePlayback::CinderPlayer::onError(const gchar* msg) const
{
    std::cerr << "Error received from GStreamer: " << msg << std::endl;
}

void CompletePlayback::CinderPlayer::onStreamEnd() const
{
    std::cout << "EOS event received from GStreamer." << std::endl;
}

void CompletePlayback::CinderPlayer::onState(GstState old) const
{
    std::cout << "GStreamer state changed from: " << old << " to: " << getState() << std::endl;
}

void CompletePlayback::setup()
{
...
void CompletePlayback::keyDown(KeyEvent event)
{
...
void CompletePlayback::fileDrop(FileDropEvent event)
{
    if (event.getNumFiles() > 0) { openVideo(event.getFile(0).generic_string()); }
}

void CompletePlayback::openVideo(const std::string& path)
{
...
void CompletePlayback::update()
{
    mPlayer.update();
}

void CompletePlayback::draw()
{
    gl::clear();
...
CINDER_APP(CompletePlayback, RendererGl, [](App::Settings* settings)
{
